...
tsline pc_sa_window fcstmean
tsline fcstmean pc_sa_window


* The naive forecast is the last value is the forecast going forward. 

gen fcstnaive = pc_sa_training
replace fcstnaive = pc_csus_sa[scalar(train_end)] if date >= tm(2010m1)
tsline fcstnaive pc_sa_window, saving(fcstnaive, replace)
...
gen fcstseasnaive = pc_nsa_training
replace fcstseasnaive = fcstseasnaive[_n-12] if date >= tm(2010m1)
tsline fcstseasnaive pc_nsa_window, saving(fcstseasnaive, replace)
...
gen fcstdrift = pc_sa_training
replace fcstdrift = fcstdrift[_n-1] + ((pc_sa_training[276] - pc_sa_training[157])/(120-1)) if date >= tm(2010m1)
tsline fcstdrift pc_sa_window, saving(fcstdrift, replace)
